...
#excl_sum
backward_model <- step(excl_glm, direction = "backward", k = log(nobs(excl_glm)), steps = 1)
...
glimpse(df)
mutate(df, 
       vituse_new = factor(vituse == "often",
                         levels = c(FALSE, TRUE),
                         labels = c("no", "yes"))) -> df
glimpse(df)
count(df, vituse_new)
...
full_join(df1, df2) |> full_join(df3) |> full_join(df4) |> full_join(df5) |> full_join(df6) |> full_join(df7)
```
...
```{r}
aic <- AIC(null_glm, excl_glm, backward_model, backward_stepwise_model, 
           forward_model, forward_stepwise_model, forward_stepwise_new)
bic <- BIC(null_glm, excl_glm, backward_model, backward_stepwise_model, 
           forward_model, forward_stepwise_model, forward_stepwise_new)
...
```{r}
bestmodel_infl <- influence(forward_stepwise_model)
glimpse(bestmodel_infl)

bestmodel_pred <- cbind(df,
                   xbeta = predict(forward_stepwise_model),
                   v = bestmodel_infl$hat)
glimpse(bestmodel_pred)

bestmodel_pred |> mutate(devresid = bestmodel_infl$dev.res,
                    stddevresid = devresid/sqrt(1 - v)) -> bestmodel_pred
glimpse(bestmodel_pred)

pplus1_bestmodel <- length(forward_stepwise_model$coefficients)
n <- nobs(forward_stepwise_model)

bestmodel_pred |> mutate(pearson = bestmodel_infl$pear.res,
                    stdpearson = pearson/sqrt(1 - v)) -> bestmodel_pred
glimpse(bestmodel_pred)

ggplot(bestmodel_pred, aes(x = xbeta, 
                      y = stddevresid, 
                      color = as.factor(lowplasma_hl))) +
  geom_point() +
  geom_hline(yintercept = c(-3, -2, 0, 2, 3), 
...
```{r}
ggplot(bestmodel_pred, aes(sample = stdpearson)) +
  geom_qq() + geom_qq_line() +
  labs(title = "Q-Q-plot standardised residuals (best AIC model)",
       x = "theoretical", y = "sample")

ggplot(bicmodel_pred, aes(sample = stdpearson)) +
  geom_qq() + geom_qq_line() +
